...
public class TicketProvider {
    ...
    public TicketProvider() {
        // Класс репозитория находится в единственном экземпляре для того,
        // чтобы не создавать несколько подключений
    ...
    public List<Ticket> getTickets(int routeNumber) throws RuntimeException {
        if (routeNumber < 1) throw new RuntimeException("неверный маршрут");
     ...
     */
    public boolean updateTicketStatus(Ticket ticket) throws NullPointerException{
        if (ticket == null) throw new NullPointerException("билет отсутствует");
