...
public class TicketRepository implements ITicketRepo {
    ...
    private TicketRepository() {
        ...
        generateTickets(1, 6, 10, date,1,2,3);
        generateTickets(2, 4, 15, date,4,5,6);
    }
    ...
    private void generateTickets(int roureNumber, int countPlaces, int price, Date date,int id,int zoneStart,int zoneStop) {
        for (int i = 1; i <= countPlaces; i++) {
            tickets.add(new Ticket(id, roureNumber, i, zoneStart, price, date, zoneStop, true));
        }
    }
}
